<?php
	...
	class question extends votable {
		...
		public function getAnswers(){
			$ret = array();
			$q = $this->pdo->query("SELECT answer.id,coalesce(sum(value),0) as vote FROM answer_vote right join answer on answer.id = answer_vote.answer where answer.question = ".$this->id." group by answer.id order by vote desc");
			if( $q->rowCount() > 0 ):
				foreach($q as $row):
